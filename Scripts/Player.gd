...
func take_damage(amount):
	health -= amount
	print("Damage taken: " , (amount) , " | Health remaining: " , (health))

func heal(amount):
	while health < max_health:   #will execute the following code as long as condition is true
		health += amount         #calls the heal function from _ready() to add 7
		if health >= max_health: #if health equals max health, while loop stops
			health = max_health
		print("Current Health: " , health)
#Every loop, add 7, then print new amount of health.
#Repeat until if statement condition stops the loop, then prints the final health value.
